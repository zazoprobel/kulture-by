...
type PageProps = { params: Promise<{ slug: string; city: string; page: string }> };

export async function generateMetadata({ params }: PageProps) {
  const { slug, city, page } = await params;
  ...
  const canonicalPath = placesCanonicalPath({ categoryDb, cityDb, page: pageNum });
...
export default async function PlacesCategoryCityPageByNumber({ params }: PageProps) {
  const { slug, city, page } = await params;
  const categoryDb = getPlacesCategoryDbFromUrlSegment(slug);
  const cityDb = getPlacesCityDbFromUrlSegment(city);
  const pageNum = Math.max(1, Number(page) || 1);
  ...
  const { items, totalCount } = await fetchPlacesListing({ categoryDb, cityDb, page: pageNum });
