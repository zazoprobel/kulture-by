...
type Place = { id: string; slug: string; name: string; city: string; category: string; rating: number | null; image_url: string | null; image_urls?: string[] | null };
type Venue = {
  id: string;
  slug: string;
  name: string;
...
export default async function HomePage() {
  ...
  try {
    ...
    const [placesQuery, venuesQuery, placesCountQuery, venuesCountQuery, citiesQuery] = await Promise.all([
      supabase.from("places").select("id,slug,name,city,category,rating,image_url,image_urls").order("rating", { ascending: false }).limit(4),
      supabase
        .from("venues")
          ...
          <div className="cards">
            {places.map((place) => (
              <Link key={place.id} href={`/places/${place.slug}`} className="card">
                <div className="thumb">
                  <Image
                    src={place.image_url || place.image_urls?.[0] || placeImages[place.category] || "https://images.unsplash.com/photo-1467269204594-9661b134dd2b?w=900"}
                    alt={place.name}
                    fill
                    unoptimized
