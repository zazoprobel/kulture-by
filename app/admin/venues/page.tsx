...
export default async function AdminVenuesPage({ searchParams }: Props) {
  ...
  let query = supabase
    .from("venues")
    .select("id,name,slug,type,city,rating,created_at", { count: "exact" })
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0 }}>Площадки</h1>
        <Link href="/admin/venues/new" style={btn}>+ Добавить</Link>
      </div>
      ...
      <table style={table}>
        <thead><tr><th>Название</th><th>Тип</th><th>Город</th><th>Рейтинг</th><th>Дата</th><th /></tr></thead>
        <tbody>
          {(data ?? []).map((item) => (
            <tr key={item.id}>
              <td>{item.name}</td>
              ...
              <td style={{ whiteSpace: "nowrap", display: "flex", gap: 8 }}>
                <Link href={`/admin/venues/${item.id}/edit`}>Редактировать</Link>
                <form action={async () => { "use server"; await deleteVenueAction(item.id); }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <Link href={`/admin/venues?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.max(1, pageNum - 1)}`}>←</Link>
        <span>Страница {pageNum} / {totalPages}</span>
        <Link href={`/admin/venues?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.min(totalPages, pageNum + 1)}`}>→</Link>
      </div>
    </div>
  );
}
