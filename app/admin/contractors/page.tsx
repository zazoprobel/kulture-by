...
import { deleteContractorAction } from "@/app/admin/actions";

type Props = { searchParams: Promise<{ q?: string; page?: string; sort?: "asc" | "desc"; saved?: string }> };

export default async function AdminContractorsPage({ searchParams }: Props) {
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0 }}>Подрядчики</h1>
        <Link href="/admin/contractors/new" style={btn}>+ Добавить</Link>
      </div>
      ...
      <table style={table}>
        ...
        <tbody>
          {(data ?? []).map((item) => (
            <tr key={item.id}>
              <td>{item.name}</td><td>{item.category}</td><td>{item.city}</td><td>{item.rating}</td>
              <td>{new Date(item.created_at).toLocaleDateString("ru-RU")}</td>
              <td style={{ whiteSpace: "nowrap", display: "flex", gap: 8 }}>
                <Link href={`/admin/contractors/${item.id}/edit`}>Редактировать</Link>
                <form action={async () => { "use server"; await deleteContractorAction(item.id); }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <Link href={`/admin/contractors?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.max(1, pageNum - 1)}`}>←</Link>
        <span>Страница {pageNum} / {totalPages}</span>
        <Link href={`/admin/contractors?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.min(totalPages, pageNum + 1)}`}>→</Link>
      </div>
    </div>
  );
}
