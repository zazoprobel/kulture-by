...
import { deleteStoryAction } from "@/app/admin/actions";
...
export default async function AdminStoriesPage({ searchParams }: Props) {
  ...
  let query = supabase
    .from("stories")
    .select("id,title,slug,city,likes,created_at", { count: "exact" })
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0 }}>Истории</h1>
        <Link href="/admin/stories/new" style={btn}>+ Добавить</Link>
      </div>
      <form style={{ display: "flex", gap: 8 }}>
        <input name="q" defaultValue={q} placeholder="Поиск по заголовку" style={input} />
      ...
      <table style={table}>
        <thead><tr><th>Заголовок</th><th>Город</th><th>Лайки</th><th>Дата</th><th /></tr></thead>
        <tbody>
          {(data ?? []).map((item) => (
            <tr key={item.id}>
              <td>{item.title}</td><td>{item.city}</td><td>{item.likes}</td>
              <td>{new Date(item.created_at).toLocaleDateString("ru-RU")}</td>
              <td style={{ whiteSpace: "nowrap", display: "flex", gap: 8 }}>
                <Link href={`/admin/stories/${item.id}/edit`}>Редактировать</Link>
                <form action={async () => { "use server"; await deleteStoryAction(item.id); }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <Link href={`/admin/stories?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.max(1, pageNum - 1)}`}>←</Link>
        <span>Страница {pageNum} / {totalPages}</span>
        <Link href={`/admin/stories?q=${encodeURIComponent(q)}&sort=${sort}&page=${Math.min(totalPages, pageNum + 1)}`}>→</Link>
      </div>
    </div>
  );
}
